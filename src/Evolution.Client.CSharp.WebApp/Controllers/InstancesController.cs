...
public class InstancesController : Controller
{
    ...
    public async Task<IActionResult> ConnectAjax(string instanceName)
    {
        ...
        try
        {
            ...
            {
                success = true,
                qrCodeBase64 = connection.Base64,
                pairingCode = connection.PairingCode,
                count = connection.Count
            });
        }
    ...
    /// <returns>Redireciona para a lista de instâncias.</returns>
    [HttpPost]
    public async Task<IActionResult> Disconnect(string instanceName)
    {
        if (string.IsNullOrEmpty(instanceName))
        {
            TempData["ErrorMessage"] = "Nome da instância não fornecido";
            return RedirectToAction(nameof(Index));
        }

        try
        {
            _logger.LogInformation("Tentando desconectar instância: {InstanceName}", instanceName);

            await _evolutionClient.Instance.LogoutInstanceAsync(instanceName);

            _logger.LogInformation("Instância desconectada com sucesso: {InstanceName}", instanceName);
            TempData["SuccessMessage"] = $"Instância '{instanceName}' desconectada com sucesso";
        }
        catch (HttpRequestException ex) when (ex.StatusCode == HttpStatusCode.NotFound)
        {
            _logger.LogError(ex, "Instância não encontrada: {InstanceName}", instanceName);
        ...
        {
            _logger.LogError(ex, "Erro ao desconectar instância: {InstanceName}", instanceName);
            TempData["ErrorMessage"] = $"Erro ao desconectar instância: {ex.Message}";
        }
    ...
    /// </summary>
    /// <param name="instanceName">O nome da instância a ser excluída.</param>
    /// <returns>Redireciona para a lista de instâncias.</returns>
    [HttpPost]
    public async Task<IActionResult> Delete(string instanceName)
    {
        ...
        try
        {
            _logger.LogInformation("Tentando excluir instância: {InstanceName}", instanceName);

            await _evolutionClient.Instance.DeleteInstanceAsync(instanceName);

            _logger.LogInformation("Instância excluída com sucesso: {InstanceName}", instanceName);
            TempData["SuccessMessage"] = $"Instância '{instanceName}' excluída com sucesso";
        }
        catch (HttpRequestException ex) when (ex.StatusCode == HttpStatusCode.NotFound)
        {
            _logger.LogError(ex, "Instância não encontrada: {InstanceName}", instanceName);
        ...
        {
            _logger.LogError(ex, "Erro ao excluir instância: {InstanceName}", instanceName);
            TempData["ErrorMessage"] = $"Erro ao excluir instância: {ex.Message}";
        }
